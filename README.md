...
一个从零实现的简单二进制反编译器：输入 ELF 可执行文件，输出接近 C 的伪代码，支持自动函数发现，LLM 漏洞分析等等（后续会添加其他功能）。
基于 Capstone 反汇编 + 自研三地址码（TAC）中间表示，端到端流水线完整可跑，支持 PE 格式二进制文件，x86/x86-64 架构等等。

## 功能特性

- **ELF 解析**：提取节区、入口点、函数符号、字符串表等信息
- **x86/x86-64 反汇编**：基于 Capstone 反汇编，支持 x86/x86-64 架构
- **TAC 中间表示**：指令提升（lift）+ 栈参数聚合，把 `mov [esp+X], v; call f` 还原成 `f(args)`，
- **C 伪代码生成**：序言/尾声折叠、负数识别、字符串地址还原为字面量
- **自动函数发现**：扫描 `call` 目标 + 序言字节模式，能挖出**未被调用的隐藏函数**
- **objdump 风格 CLI（wjdump）**：`-f/-h/-d/-D/-s/-t/-j/--start-address/--stop-address/-M`，带 `<name@plt>` 符号标注，排版对齐 objdump
...
```bash
pip install -r requirements.txt # pyelftools, pefile, capstone, openai
```

## 快速开始

```bash
反编译 main 函数（指定地址 + 指令条数）
# python demo_decompile.py test/ret2text 0x8048648 30

自动发现所有函数并逐个反编译
# python demo_decompile_all.py test/ret2text
```
...
```bash
从 main 符号/入口点展开调用图（深度≤3、节点≤20）并交给大模型研判
...
对应的 objdump 选项：`-f -h -d -D -s -t -j --start-address --stop-address -M`。
...
输入二进制 (ELF / PE)
      ...
      ▼
┌───────────────────────────────────────────────────────────────┐
│ loader ── guess_loader() → ElfLoader / PeLoader               │
...
┌───────────────────────────────────────────────────────────────┐
│ ir ── Lifter                                                │
│   汇编 → 三地址码 TAC，栈参数聚合: mov [esp+X],v → CALL(args)│
│   产出: IRInst[] (ASSIGN / CALL / RET / UNKN)               │
└───────────────────────────────────────────────────────────────┘
      │  IRInst[]
      ▼
┌───────────────────────────────────────────────────────────────┐
...
CLI 入口:
  ...
  wjdump.py <-f|-h|-d|-D|-s|-t> <binary>     → objdump 风格查看/反汇编
...
- **TAC 而非 VEX/P-code**：目标是可读性而非精确仿真，一条汇编通常对应一条 IR，标志位、栈指针副作用按需建模。
...
- PE 支持未完善（间接调用、IAT 解析待做）
- 条件跳转/循环未建模，分支代码输出为 `// 未实现` 注释
- 变量未命名（暂用寄存器名 eax/esp），无类型恢复
- 序言模式扫描有重叠误报（同一函数被命中多次），待做覆盖去重

## 下一步规划

- [ ] 函数发现去重（已覆盖区间跳过）
- [ ] CFG 构建 + 控制结构恢复（if/while）
